            ...
            <div class="flex w-full h-4/5">
                <form action="{{route('posts.update',compact('post'))}}" method="post" enctype="multipart/form-data" class="w-full h-full flex flex-row-reverse">
                    ...
                    <div class="w-1/2 h-full flex flex-col items-end pr-20 relative">
                        ...
                        <div class="w-5/6 h-auto flex flex-row-reverse justify-between pt-4 mb-6">
                            <label for="price" class="font-semibold ml-5">: date</label>
                            <input type="date" name="date" value="{{$post->date}}" id="date" class="w-2/5 h-8 rounded outline-0 p-2 border border-gray-400">
                            @error('date')
                            <p class="text-red-700">{{$message}}</p>
                            @enderror
                        </div>
